...
const formInputClasses = "block w-36 px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500 sm:text-sm disabled:bg-gray-100 disabled:text-gray-500 disabled:cursor-not-allowed";
...
export default function ApiTest() {
  ...
  return (
    <div class="space-y-6 p-4 md:p-6">
      ...
      {/* Wrap the accordion in the shared container style */}
      <div class={sectionContainerClasses}>
        {/* Accordion Root - Minimal styling needed here */}
        <div {...postAccordionApi().getRootProps()}>
          {/* Accordion Item */}
          <div {...postAccordionApi().getItemProps({ value: "post-request" })}>
            {/* Trigger Container (h3 is semantically appropriate) */}
            <h3>
              {/* Apply updated trigger classes that mimic h2 */}
            ...
            {/* Apply updated content classes */}
            <div {...postAccordionApi().getItemContentProps({ value: "post-request" })} class={accordionContentClasses}>
              {/* Content: Label, Textarea, Button (Uses existing shared constants) */}
              <div>
